...
def Epsilon_works(Coordinates, ray_freq=0.5, Norm=True):
    ...
    #
def generate_Empty_TPL(freq,file_name):
...
def Epsilon(Coordinates, ray_freq=0.4, Norm=True):
    ...
    k1 = (RocketWidth) / (RocketZHigh - (RocketZHigh + RocketConeHeight))
    b = RocketXLoc - k1 * (RocketZHigh + RocketConeHeight)
    ...
    exp = np.where(((z >= RocketZHigh) & (z <= (RocketZHigh + RocketConeHeight)) & (
            np.abs(x - RocketXLoc) <= k1 * z + b - RocketXLoc)), par, exp)
...
def calc_rasp(par):
    ...
    if Only_rocket == False:
        def_name = f'rocket_fakel_freq_{freq}_k_{kk}_ang_'.replace('.','_')
        X_boundary = 5  # z
        Y_boundary = 7  # x
        Xmin = -5  # z
        Ymin = -15  # x
        ...
        for Yi, Y in enumerate(y):
            for Xi, X in enumerate(x):
                ...
                if k % 10000 == 0:
                    ...
                    spend_time = (dt.datetime.now()-t)
                    rest_time = (N/k-1)*spend_time
                    clear_cons()
                    print(name,done*100,'% rest time',rest_time )
                X1, Z1 = X * np.cos(angle) + Y * np.sin(angle), -X * np.sin(angle) + Y * np.cos(angle)
                if Only_rocket == True:
                    a = set_a(Epsilon_no_fakel([X1, Z1],ray_freq=freq), Xi, Yi)
                else:
                    a = set_a(Epsilon([X1, Z1],ray_freq=freq), Xi, Yi)
...
if __name__ == '__main__':
    if Empty == True:
        generate_Empty_TPL(4,'4_Empty')
    else:
        Pool(potoki).map(calc_rasp, par)
        Pool(potoki).close()
        for fr in freq_l:
            file = path + f'run freq {fr}.bat'
            f = open(file, 'w')
            f.close()
            for ang in angles:
                f = open(file, 'a')
                if Only_rocket == False:
                    def_name = f'rocket_fakel_freq_{fr}_k_{kk}_ang_'.replace('.','_')
                elif Only_rocket == True:
                    def_name = f'rocket_{fr}_k_{kk}_ang_'.replace('.','_')
                name = def_name + str(str(np.round(np.degrees(ang), 0))[:-2]) + '.TPL'
                s = f'start tmc_rth.exe {name}\n'
                f.write(s)
                f.close()
