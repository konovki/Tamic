...
#TOPOLOGY
 ...
 END_B
! BLOCK 2;
!  RECT_STAT W_type1 ; 0.0; l3+l_r; -a1/2-2*W_delta; -a1/2-W_delta;
! END_B
! BLOCK 3;
!  RECT_STAT W_type1 ; 0.0; l3+l_r; -a1/2+a2-2*W_delta; -a1/2+a2-2*W_delta+0.1;
! END_B
...
! T  1; L_input; h1+a1/2+W_tip;
! T  2; L_input-2*W_delta; h1+a1/2+W_tip;
! T  3; L_input-2*W_delta; h1+a1/2+W_tip;
 T  101; (L_waveg)/2+x_shift/2-d_a1/2; (W_waveg)/2-(h1+h2+h3+h4)/2 + (W_wavegPlus)/2;
! T  102; L_input-2*W_delta; h1+a1/2+W_tip+1*a2;
! T  103; L_input-2*W_delta; h1+a1/2+W_tip+1*a2;
 T  4; 0.0; 0.0;
 T  5; 0.0; 0.0;
 T  6; 0.0; 0.0;
 T  7; 0.0; 0.0;
...
! T  10; 0.0; 0.0;
 ...
 T 1033; L_waveg-d2/2; (W_waveg)/2-h3/2+4*(W_delta);
#END_LINK
...
#OUTPUT
 ...
 FIELDS;
 TOPOLOGY;
 FIELD_DISTRIBUTION_M rup_L21; W_freq; 490; 495;
#END_OUTPUT
